...
export async function getPlanAccess(userId, serviceRoleKey) {
  const [unlockResponse, generationResponse] = await Promise.all([
    fetch(`${SUPABASE_URL}/rest/v1/mks_unlocks?user_id=eq.${encodeURIComponent(userId)}&select=unlocked_at&limit=1`, {
      headers: serviceHeaders(serviceRoleKey),
    }),
    fetch(`${SUPABASE_URL}/rest/v1/mks_goal_generations?user_id=eq.${encodeURIComponent(userId)}&select=goal_data&limit=1`, {
      headers: serviceHeaders(serviceRoleKey),
    }),
  ]);
  if (!unlockResponse.ok || !generationResponse.ok) throw new Error('Could not verify plan access');
